...
import type { Country } from "@/components/templates/onboarding-flow";
...
export default function SelectCountry({ onBack, onNext }: Props) {
        ...
        >
          {filteredCountries.map((ct) => (
            <RadioGroup.Item
              ...
              style={{
                backgroundColor: colors.inputBackground,
                borderWidth: selectedCode === ct.code ? 2 : 0,
                borderColor: colors.textPrimary,
              }}
            >
              <View className="flex-row items-center gap-3.25">
                <Text className="text-[30px]">{ct.flag}</Text>
                <Text
                  className="text-body-sm leading-3.75"
                  style={{

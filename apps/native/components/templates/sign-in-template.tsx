...
import { signinSchema, type SignInType } from "@cashory/schema";
  ...
  const handleSignIn = async (data: SignInType) => {
    ...
    if (!parsed.success) {
      parsed.error.issues.forEach((err) => {
        if (err.path[0]) {
          setError(err.path[0] as keyof SignInType, {
            type: "manual",
            message: err.message,
          });
        }
    ...
    try {
      ...
      setAuthError(
        getAuthErrorMessage(error as Error, "Invalid password or email"),
      );
    }
  };
